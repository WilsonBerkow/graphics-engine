...
pub fn save_ppm(image: &Screen, filename: &str) -> io::Result<()> {
...
pub fn save_png(image: &Screen, filename: &str) -> io::Result<()> {
...
pub fn display_image(image: &Screen) -> io::Result<()> {
...
pub fn write_image(file: &mut File, image: &Screen) -> io::Result<()> {
    let start = Instant::now();
    ...
    if let Err(e) = bufwriter.write(image.as_bytes()) {
        return Err(e);
    }
    if DEBUG {
        let elapsed = start.elapsed();
